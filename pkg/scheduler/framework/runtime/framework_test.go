...
func TestRecordingMetrics(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			action: func(ctx context.Context, f framework.Framework) {
				f.RunPlacementGeneratorPlugins(ctx, state, nil, []*fwk.PlacementInfo{{}})
			},
...
func TestRunPlacementGeneratorPlugins(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Calls plugins with placements generated by previous plugins",
			plugins: []simplifiedGeneratorPlugin{
				{
					assertState: func(t *testing.T, parentPlacements []*fwk.PlacementInfo) {
		...
		{
			name: "Does not call plugins with placements that don't match any nodes",
			plugins: []simplifiedGeneratorPlugin{
				{
					selectors: []*v1.NodeSelector{
						makeNodeSelector(map[string]string{"k1": "v1"}),
				...
				},
				{
					assertState: func(t *testing.T, parentPlacements []*fwk.PlacementInfo) {
						if len(parentPlacements) != 1 {
							t.Fatalf("Expected 1 parent placement from the previous plugin, got %d", len(parentPlacements))
						}
					},
				},
			},
		...
		{
			name: "Only matches selectors against placements from previous plugins",
			plugins: []simplifiedGeneratorPlugin{
				{
					selectors: []*v1.NodeSelector{
						makeNodeSelector(map[string]string{"k1": "v1"}),
				...
				{
					selectors: []*v1.NodeSelector{
						// In isolation this matches node1 and node3,
						// but the previous plugin placements don't match node3.
						// Therefore the resultint placement only contains node1
		...
		},
		{
			...
			plugins: []simplifiedGeneratorPlugin{
				...
				{
					// Merged with the previous selector the resulting placements should be:
					// - k1:v1 AND k2:v1
					// - k1:v2 AND k2:v1
					selectors: []*v1.NodeSelector{
						makeNodeSelector(map[string]string{"k2": "v1"}),
		...
		{
			name: "Merges multiple selector terms",
			plugins: []simplifiedGeneratorPlugin{
				{
					selectors: []*v1.NodeSelector{
						makeNodeSelector(map[string]string{"k1": "v1"}, map[string]string{"k1": "v2"}),
				...
				},
				{
					selectors: []*v1.NodeSelector{
						// Merged with the previous selector the resulting placements should be:
						// - (k1:v1 AND k2:v1) OR (k1:v2 AND k2:v1)
						// - k1:v3 AND k2:v1
						// - (k1:v1 AND k2:v2) OR (k1:v2 AND k2:v2)
						// - k1:v3 AND k2:v2
						makeNodeSelector(map[string]string{"k2": "v1"}),
						makeNodeSelector(map[string]string{"k2": "v2"}),
					},
				},
			},
			nodes: []*v1.Node{
				st.MakeNode().Name("node1").Label("k1", "v1").Label("k2", "v1").Obj(),
				st.MakeNode().Name("node2").Label("k1", "v1").Label("k2", "v2").Obj(),
				st.MakeNode().Name("node3").Label("k1", "v2").Label("k2", "v1").Obj(),
				st.MakeNode().Name("node4").Label("k1", "v2").Label("k2", "v2").Obj(),
				st.MakeNode().Name("node5").Label("k1", "v3").Label("k2", "v1").Obj(),
				st.MakeNode().Name("node6").Label("k1", "v3").Label("k2", "v3").Obj(),
			},
			initialPlacements: [][]string{
		...
		{
			name: "Handles plugins returning same selectors as their parents",
			plugins: []simplifiedGeneratorPlugin{
				{
					selectors: []*v1.NodeSelector{
						makeNodeSelector(map[string]string{"k1": "v1"}),
